--[[
This file is part of MiCaWebUI.

MiCaWebUI is free software; you can redistribute it and/or modify
...
function index()
  function luci.dispatcher.authenticator.arduinoauth(validator, accs, default)
    ...
    if user then
      ...
      elseif #pass == 64 then
        ...
        uci:load("micabridge")
        local stored_encrypted_pass = uci:get_first("micabridge", "bridge", "password")
    ...
    luci.http.header("WWW-Authenticate", "Basic realm=\"micabridge\"")
  ...
  local uci = luci.model.uci.cursor()
  uci:load("micabridge")
  local secure_rest_api = uci:get_first("micabridge", "bridge", "secure_rest_api")
...
function storage_send_request()
  ...
  local uci = luci.model.uci.cursor()
  uci:load("micabridge")
  local socket_timeout = uci:get_first("micabridge", "bridge", "socket_timeout", 5)
...
function board_plain_socket()
  ...
  if not sock then
    code = code or ""
    msg = msg or ""
    http_error(500, "Could not connect to MiCaServer " .. code .. " " .. msg)
    return
  end
